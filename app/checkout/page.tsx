...
export default function CheckoutPage() {
            ...
            <div className="min-h-screen bg-slate-50 py-8">
                <div className="container mx-auto px-4">
                    <div className="flex flex-col items-center justify-center py-20">
                        ...
                        <p className="text-slate-500 mb-6">Buyurtma berish uchun mahsulot qo'shing</p>
                        <button
                            onClick={() => router.push('/')}
    ...
    const handleSubmit = async () => {
        if (!selectedAddressId) {
            showToast('Iltimos, manzilni tanlang', 'error');
            return;
        }
    ...
    return (
        <div className="min-h-screen bg-slate-50 py-8">
            <div className="max-w-7xl mx-auto px-6">
                <h1 className="text-3xl font-bold text-slate-900 mb-8">Buyurtmani rasmiylashtirish</h1>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-lg p-6">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h2 className="text-xl font-bold text-slate-900">Yetkazib berish manzili</h2>
                            </div>
                            <AddressSelector
                        ...
                        <div className="bg-white rounded-lg p-6 sticky top-20">
                                ...
                                {items.map((item) => (
                                    <div key={item.id} className="flex gap-3 text-sm">
                                        <div className="flex-1">
                                            <p className="text-slate-900 line-clamp-1">{item.variant.product.name}</p>
                                            <p className="text-slate-500">{item.quantity} x {(item.variant.discountPrice || item.variant.price).toLocaleString('uz-UZ')} so'm</p>
                                        </div>
                                        <p className="font-semibold text-slate-900">
                                            {((item.variant.discountPrice || item.variant.price) * item.quantity).toLocaleString('uz-UZ')} so'm
                                        </p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="border-t border-slate-200 pt-4 mb-6">
                                <div className="flex justify-between text-xl font-bold text-slate-900">
                                    <span>Jami:</span>
                                    <span className="text-[#7000ff]">{finalTotal.toLocaleString('uz-UZ')} so'm</span>
                                </div>
                            </div>
